<#
    ...
    .DESCRIPTION
        ...
        Option 1: Use and configure the Azure Run AS Account as Service Principal, and give it the following permissions: user.read.all reports.read.all groupmember.read.all and UserAuthenticationMethod.ReadWrite.All
        Option 2: Create an app registration that has application: user.read.all reports.read.all groupmember.read.all and UserAuthenticationMethod.ReadWrite.All
...
# Recommended option - use Automation Account Service Principal
$script:appCert = Get-AutomationCertificate -Name "AzureRunAsCertificate"
# Option2 - use App Registration Secret
...
function get-authToken {
    <#
        .SYNOPSIS
            Requests a Application based token grant form Microsoft Graph API with either Certifcate or Secret
    #>
    ...
    $script:TokenExpiresOnUTC = $response.ExpiresOn.UtcDateTime
    return $authToken
}#endfunction
...
foreach ($user in $allUsersToRegisterWithMobile) {
    ...
    if ($RefreshTokenTime -ge $TokenExpiresOnUTC) {
    ...
    try {
        $response = Invoke-MSGraphOperation -Post -APIVersion $graphVersion -Headers $authTokenApp -Body $JSON -Resource "users/$user/authentication/phoneMethods" -ErrorAction Stop
